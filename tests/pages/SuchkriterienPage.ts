...
export class SuchkriterienPage {
    ...
    readonly isbnInput = this.page.getByRole('textbox', {
        name: 'z.B. 978-3-897-22583-1',
    });
    readonly titleInput = this.page.getByRole('textbox', {
        name: 'z.B. JavaScript Patterns',
    });
    readonly radioEPUB = this.page
        .locator('div')
        .filter({ hasText: /^EPUB$/ })
        .getByRole('radio');
    readonly radioHARDCOVER = this.page
        .locator('div')
        .filter({ hasText: /^HARDCOVER$/ })
        .getByRole('radio');
    readonly radioPAPERBACK = this.page
        .locator('div')
        .filter({ hasText: /^PAPERBACK$/ })
        .getByRole('radio');
    readonly lieferbarSelect = this.page.getByRole('combobox');
    readonly checkboxJavaScript = this.page
        .locator('div')
        .filter({ hasText: /^JavaScript$/ })
        .getByRole('checkbox');
    readonly checkboxTypeScript = this.page
        .locator('div')
        .filter({ hasText: /^TypeScript$/ })
        .getByRole('checkbox');
    readonly checkboxJava = this.page
        .locator('div')
        .filter({ hasText: /^Java$/ })
        .getByRole('checkbox');
    readonly checkboxPython = this.page
        .locator('div')
    ...
    async selectSchlagwort(
        wort: 'JavaScript' | 'TypeScript' | 'Java' | 'Python',
    ) {
        switch (wort) {
            case 'JavaScript':
    ...
    async selectSchlagwoerter(
        worte: ('JavaScript' | 'TypeScript' | 'Java' | 'Python')[],
    ) {
