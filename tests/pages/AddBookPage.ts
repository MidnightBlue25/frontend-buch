...
export class AddBookPage {
  ...
  readonly sidebarButton = this.page.getByRole('button').filter({ hasText: /^$/ }).first();
  readonly newIsbnButton = this.page.getByRole('button', { name: 'Neue ISBN erzeugen' });
  readonly isbnInput = this.page.getByRole('textbox', { name: 'Generierte ISBN' });
  readonly copyButton = this.page.getByRole('button', { name: '📋 Kopieren' });
  readonly closesidebarButton = this.page.getByRole('button', { name: 'Close' });
